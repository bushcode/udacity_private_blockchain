...
class Blockchain {
	...
	_addBlock(block) {
		...
		return new Promise(async (resolve, reject) => {
			...
			//increament current block height
			self.height += 1;

			//safe guard check to ensure block was added correctly
		...
		return new Promise(async (resolve, reject) => {
			if (self.height > 0) {
				for (var i = 1; i <= self.height; i++) {
					...
					if (!validation) {
						console.log('error occured validating block data...');
					} else if (block.previousBlockHash != self.chain[i - 1].hash) {
						console.log('error with previous hash...');
					}
				}
				if (errorLog) {
					resolve(errorLog);
				} else {
					resolve('Chain validated.');
				}
			} else {
